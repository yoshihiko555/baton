...
| `PaneID string` | `PaneID int` | 型変更（後述） |
...
| （なし） | `CandidatePaneIDs []int` | 内部フィールド（JSON 出力対象外） |
...
#### PaneID の string → int 変更理由

WezTerm CLI（`wezterm cli list --format json`）が pane ID を整数として出力する。v1 では文字列として扱っていたが、以下の理由で整数型に変更する。

- 型変換（`strconv.Atoi`）の散在を防ぐ
- `DetectedProcess.PaneID` との比較が直接可能になる
- 外部出力（DTO）では `strconv.Itoa` で文字列に変換する（後述）
...
| `PaneID` | `int` | 関連 WezTerm ペイン ID（0 = 未関連） |
